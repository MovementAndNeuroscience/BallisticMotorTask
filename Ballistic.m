%% Modified Sept 2013 ->    Added variable 'inOffset' that adds to the input data effectively shifting
%%                          the curve. This variable is not saved in the experiment files to avoid running
%%                          into version problems. It is however saved in the config file. The shifting
%%                          occurs in function 'draw' and affects all types of inputs.
...
classdef Ballistic < handle
    ...
    properties (Constant, Hidden)
        ...
        MINIMUMPEAKDETECTIONLEVEL = 1.25; % in input device units (v or pixels)
        ...
        DECELERATIONDURATION = 20; % in samples
...
time = time - min(time) + mean(diff(time));
            ...
            % Shifting the data by offset
            data = data + this.inOffset;
...
time = time + v;
        ...
        function startExperiment(this)
            ...
            if (this.trialTime <= 0)
                ...
                set(this.trialTimer, 'TasksToExecute', max(floor(this.trialTime), 1));
    ...
    methods (Access = private)
        ...
        function setCallbacks(this)
            ...
            set(this.mainGUI.maskTimeBox, 'Callback', @this.maskTimeBoxChange);
            set(this.mainGUI.inOffsetBox, 'Callback', @this.inOffsetBoxChange);
            ...
            set(this.mainGUI.inOffsetBox, 'String', this.inOffset);
            ...
            set(this.mainGUI.inOffsetBox, 'UserData', this.inOffset);
        end
        ...
        function inOffsetBoxChange(this, box, ~)
            this.inOffset = Ballistic.checkNumberBox(box);
        end
    end
